//intital variables - I tried to take the feedback from sketch 2 and keep things more local
...
function draw() {
 ...
 background (0);
  ...
  burn -= 0.0009;       // this will allow the flame to slowly decrease =
  ...
  drawBase();
  if (burn < 0.25) {
  createSmoke(width/2, height/2-140); //start smoke particles when flame done
  }
  updateSmoke();
  drawSmoke();
  fadeFabric();
...
function drawTable() {
  ...
  let fabric = fadeFabric(burn);
  
   fill(235, 225, 240, 120,fabric);
   rect(-150, -10, 300, 150, 20); //back fabric
   fill(200,230,255,fabric);
   ellipse(0,-10,300,25,);//top 
   fill(200,230,255,15,fabric);//folds in the fabric
   ellipse(0,40,15,110)
   ellipse(40,40,15,110)
   ellipse(-40,40,15,110)
   ellipse(80,40,15,100)
   ellipse(-80,40,15,100)
   ellipse(120,40,15,90)
   ellipse(-120,40,15,90)
...
function drawCandle() {
  ...
  push(); // push pop act like a container for the draw style changes - like fill or scale
    ...
    translate(width/2, height/2 - 50); //translate changes the origin from (0,0) to (x,y) -> doing width/2 height/2 places the object in the center of the acreen and allows for the drawing to asapt even if the screen size changes. 
...
function drawBase() {
    ...
    fill(192,192,192);
    ellipse(0,-100,90,25);
    rect(-5,-100,10,250,5);//holder
    rect(-3,-125,8,25,2);//prongs
    rect(-40,-125,8,25,2);
    rect(35,-125,8,25,2);
    ellipse(0,150,90,25);
...
function drawFlame(x,y,burnValue) {
    ...
    fill(230,255,255,alpha * 0.15);
    ellipse(0,-130,size *15.5,size *15.5,5); //larger glow 
...
function fadeFabric(burnValue) {
  
   let alpha = map(burnValue, 0.5, 0, 255, 40);
   alpha = constrain(alpha, 40, 255); //constrain does the inverse of max

  return alpha;
}


function fadeBlack(burnValue) { //darkens the overall scene as the candle goes out - based on class feedback

  push();

    let alpha = map(burnValue, 0.3, 0, 0, 255);
...
class Smoke{
  ...
  constructor(x,y) {
    ...
    this.vx = random(-1,1) //setting up velocity control
    this.vy = random(-0.5,-1) //used the same values as flicker but made y both negative so they go up
    
    this.size = random(15,20); // controls the size of particles
   ...
   ellipse(this.x,this.y-50,this.size*1.3,this.size); //life decreasing which controls opacity so the particles die out
